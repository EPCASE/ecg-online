...
| Progression | `localStorage` déjà utilisé par les parcours existants | Isoler la clé `edu-ecg:introduction:v1` |
...
flowchart LR
    ...
    U --> E["Moteur déterministe<br>edu-ecg-core.js"]
    U --> S["Progression locale<br>edu-ecg-store.js"]
    A["Assets approuvés"] --> R
    M["Asset absent"] --> P["Placeholder explicite<br>à valider"]
```

Le backend valide les identifiants, les types, les phases, les statuts, les chemins d’actifs, l’unicité des activités et l’absence d’indices dans un test autonome avant d’exposer le contenu.
...
La correction est locale et déterministe. Aucun LLM n’intervient.
...
python -m unittest tests.test_edu_ecg_routes tests.test_pathway_routes tests.test_collector_metrics
```
...
7. recharger la page et vérifier la reprise de progression.
